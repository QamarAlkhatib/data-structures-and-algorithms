...
const extractAbilities = (arr) => {
  ...
  let newArray =[];
  arr.map((element) =>{
    newArray.push(element.ability.name);
  });
  return newArray;
};
...
const extractStats = (arr) => {
  ...
  let newArray =[];
  arr.map((element) =>{
    let name = "name:" + `${element.stat.name} `;
    let total = "total:" + `${element.effort + element.baseStat}`;
    newArray.push(name,total);
